...
public class AcTestSet {
	...
	public List<TestResult> isOk(ServiceResourceResolverService serviceResourceResolverService) throws RepositoryException, LoginException {
		...
		try {
			...
			cleanup(authorizableToTest, testuser, testUserResolver, serviceResourcerResolver);
		}
		return resultList;
	}

	private void cleanup(Authorizable authorizableToTest, User testuser, ResourceResolver testUserResolver, ResourceResolver serviceResourcerResolver) {
		...
		if (serviceResourcerResolver != null) {
			try {
				...
				// no temporary testuser is created when a systemuser is tested via impersonation
				if (authorizableToTest instanceof Group && testuser != null) {
